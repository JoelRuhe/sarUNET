...
def conv3d(x, fmaps, kernel, dataformat, activation, param=None, lrmul=1):
    if dataformat == "NCDHW":
        w = get_weight([*kernel, x.shape[1].value, fmaps], activation, param=param, lrmul=lrmul)
        return tf.nn.conv3d(x, w, strides=[1, 1, 1, 1, 1], padding='SAME', data_format=dataformat)
    else:
        w = get_weight([*kernel, x.shape[4].value, fmaps], activation, param=param, lrmul=lrmul)
        return tf.nn.conv3d(x, w, strides=[1, 1, 1, 1, 1], padding='SAME', data_format=dataformat)


def conv3d_transpose(x, fmaps, kernel, dataformat, activation, param=None, lrmul=1):
    if dataformat == "NCDHW":
        ...
        return tf.nn.conv3d_transpose(x, w, output_shape, strides=[2, 2, 2], padding='SAME', data_format=dataformat)
    else:
        print(x.shape, 'X.SHAPE')
        print(fmaps, 'FMAPS')
        output_shape = tf.stack([x.shape[0].value, int(x.shape[1].value * 2), int(x.shape[2].value * 2), int(x.shape[3].value * 2), fmaps])
        w = get_weight([*kernel, x.shape[4].value, fmaps], activation, param=param, lrmul=lrmul)
        w = tf.transpose(w, perm=[0, 1, 2, 4, 3])
...
def maxpool3d(x, pool_size, strides, data_format, padding):
    return tf.nn.max_pool3d(x, pool_size, strides, padding, data_format)
